...
@section('title', __('erp.edit_user'))

@section('content')

{{-- Hero Section --}}
<section id="edit-user-hero" class="hero section pattern-bg flex items-center justify-center text-center pt-20">
    <div class="hero-content max-w-3xl mx-auto">
        <h1 class="hero-title font-bold mb-6">{{ __('erp.edit_user') }}</h1>
        <p class="text-lg md:text-xl mb-8">{{ __('erp.edit_user_subtitle') }}</p>
...
<section id="edit-user" class="section bg-white">
    <div class="auth-container dark:bg-gray-900">
        <div class="auth-card">
            <div class="auth-header">
                ...
                <h1 class="auth-title">{{ __('erp.edit_user') }}</h1>
                <p class="auth-subtitle">
                    {{ __('erp.edit_user_form_subtitle') }}
                </p>
            </div>
            ...
            <form class="auth-form" method="POST" action="{{ route('users.update', $user) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                        ...
                        <div class="relative mb-4">
                            <div class="w-32 h-32 rounded-full border-4 border-gray-300 bg-gray-100 flex items-center justify-center overflow-hidden">
                                @if($user->profile_image)
                                    <img id="profile-image-preview" src="{{ Storage::disk('public')->url($user->profile_image) }}" 
                                         alt="Profile Preview" class="w-full h-full object-cover">
                                    <i class="fas fa-user text-gray-400 text-4xl hidden" id="profile-image-icon"></i>
                                @else
                ...
                <div class="form-group">
                    ...
                    @error('password')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                    <p class="text-xs text-gray-500 mt-1">{{ __('erp.password_leave_blank') }}</p>
                </div>
                ...
                <div class="form-group">
                    <div class="input-wrapper">
                        ...
                        <select name="status" id="status" class="form-input" required>
                            ...
                            <option value="active" {{ old('status', $user->status->value) == 'active' ? 'selected' : '' }}>{{ __('erp.active') }}</option>
                            <option value="inactive" {{ old('status', $user->status->value) == 'inactive' ? 'selected' : '' }}>{{ __('erp.inactive') }}</option>
                            <option value="suspended" {{ old('status', $user->status->value) == 'suspended' ? 'selected' : '' }}>{{ __('erp.suspended') }}</option>
                        </select>
